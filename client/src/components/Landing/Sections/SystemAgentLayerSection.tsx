...
const SystemAgentLayerSection: FC = memo(() => {
  const t = useT();
  const prefersReduced = useReducedMotion() ?? false;
  const tr = (k: string) => (t(k as any) as unknown as string) ?? '';
  const Icon = getIcon('agents' as any);
  const abVariant = getVariantForKey('cta', 'base');

  const badges = t('system.layers.agents.badges', { returnObjects: true } as any) as unknown as
    | string[]
    | undefined;
  ...
  return (
    <LandingSection
      id="system-agents"
      ariaLabel={tr('system.layers.agents.title') || 'Agenten‑Workflows Ebene'}
      className="bg-transparent"
      bleed={false}
      dataSection="system-agents"
      data-ai-section="system-agents"
      data-ai-title={tr('system.layers.agents.short') || 'Agenten'}
    >
      <motion.div
      ...
      >
        <HeadingBlock align="center">
          <SectionHeading
            title={tr('system.layers.agents.title')}
            subtitle={tr('system.layers.agents.desc')}
            align="center"
          />
        </HeadingBlock>
      </motion.div>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-transparent p-6 ring-1 ring-inset ring-white/10">
        <div className="mb-4 inline-flex items-center gap-2">
          ...
          <span className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
            {tr('system.layers.agents.short')}
          </span>
        </div>
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
          {tr('system.layers.agents.title')}
        </h3>
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
          {tr('system.layers.agents.desc')}
        </p>
        {hasBadges && (
          <ul
        ...
        <div className="mt-4">
          <a
            ...
            data-analytics={`system_agents_link`}
            aria-label={tr('system.layers.agents.link') || 'Mehr über Agenten erfahren'}
            onClick={() =>
              track({
                name: `click_system_agents_link`,
                props: { variant: abVariant, section: 'system_agents' },
              })
            }
          >
            {tr('system.layers.agents.link') || 'Mehr über Agenten erfahren'}
          </a>
        </div>
      </div>
    </LandingSection>
...
export default SystemAgentLayerSection;
