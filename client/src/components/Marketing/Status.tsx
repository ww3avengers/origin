...
const Status: FC = () => {
  ...
  const title = safeT(t as any, 'pages.status.title', 'Status');
  const description = safeT(
    t as any,
    'pages.status.description',
    'Systemstatus, Verfügbarkeit und Inzident-Historie von SIGMACODE AI.',
  );

  return (
    <>
      <Meta title={title} description={description} image="/assets/og/status.svg" />
      <TopNav />
      <a
        href="#main-content"
      ...
      >
        ...
        <div className="text-gray-400">Status-Übersicht folgt in Kürze.</div>
      </main>
      <FooterSection />
    </>
  );
